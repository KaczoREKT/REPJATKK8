...
namespace Exercise6
{
    public static class LinqTasks
    {
        ...
        public static IEnumerable<Emp> Task4()
        {
            ...
            var querySyntax =
                from e in Emps
                where e.Salary == (from e2 in Emps
                    select e2.Salary).Max()
                select e;
            IEnumerable<Emp> result = querySyntax;
            return result;
        }
        ...
        public static IEnumerable<object> Task5()
        {
            ...
            IEnumerable<object> result = querySyntax.ToList();
            return result;
        }
        ...
        {
            var methodSyntax =
                Emps.Where(e => e.Job == "Frontend programmer")
                    .OrderByDescending(e => e.HireDate)
                    .FirstOrDefault();

            var methodSyntaxOptional =
                Emps.Where(e => e.Job == "Frontend programmer")
                    .MaxBy(e => e.HireDate);
        ...
        {
            var methodSyntax =
                Emps.Select(e => new { e.Ename, e.Job, e.HireDate })
                    .Union(new[]
                    {
        ...
        {
            var methodSyntax =
                Depts.Where(d => Emps.Count(e => e.Deptno == d.Deptno) > 1)
                    .Select(d => new
                    {
                        name = d.Dname,
            ...
            var querySyntax =
                from d in Depts
                let employeeCount = (from e in Emps where e.Deptno == d.Deptno select e).Count()
                where employeeCount > 1
                select new
                {
                    name = d.Dname,
                    numOfEmployees = employeeCount
                };
        ...
        {
            var methodSyntax =
                Depts.Where(d => Emps.Count(e => e.Deptno == d.Deptno) == 5 || !Emps.Any(e => e.Deptno == d.Deptno))
                    .OrderBy(d => d.Dname);
